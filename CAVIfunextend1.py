...
##Check any entry for inv_w_1 in the ELBOs 
...
def usumhlk(Y, X, beta_1, rho_mu, inv_sigma2_1, T, t, s): 
    ...
    for k in np.arange(t+1,T,1):
        ...
        ind.append(inv_sigma2_1[k] * np.dot(Y[:,index] - np.dot(X, (beta_1[index,]).T), rho_mu[index, k]) * rho_mu[t,k]) #append adds to the arrary
    ...
    for t in np.arange(T):
        for s in np.arange(p):
            ...
            if t < (T - 1):
                ...
                B = np.sum(inv_sigma2_1[t+1:] * rho_mu[t,t+1:] * uku , axis = 1)    
                C = usumhlk(Y = Y, X = X, beta_1 = beta_1, rho_mu = rho_mu, inv_sigma2_1 = inv_sigma2_1,
                            T = T, t = t, s = s)
                D = np.sum((inv_sigma2_1[t+1:] * rho_2[t, t+1:])[:,np.newaxis] * ut_s, axis = 0)
...
def kprodaccu(Y, X, beta_1, rho_mu, T):
    ...
    cumtot = np.concatenate([np.zeros(T-len(total)),np.cumsum(total)])
    return (np.array([cumtot]))
                

                             
def sigma2_updateu(Y, X, nu, T, n, gamma_1, tau_1, rho_mu, rho_2, inv_w_1, beta_1, beta_2):
    """"
	Purpose
	-------
    ...
    A = tau_1 * (1 / 2) + tau_1 * (1 / 2) * np.sum(rho_2, axis= 0)
    B = (1 / 2) * U_2 
    C = (1 / 2) * np.concatenate([np.zeros(1), np.dot(U_2[:T-1], rho_2[:T-1,1:])]) 
    D = kprodaccu(Y = Y, X = X, beta_1 = beta_1, rho_mu = rho_mu, T = T)
    E = np.concatenate([np.zeros(1), np.sum(U[:,1:] * np.dot(U[:,1:], rho_mu[:T-1,1:]), axis = 0)])
    
    b_star_sigma2 = A + B + C + D - E 
...
def rho_updateu(Y, X, beta_1, beta_2, inv_sigma2_1, tau_1, T, rho_mu):
    ...
    #U_2 = <U_t, U_t> for all t
    U_2 = np.sum(Y**2, axis = 0) - 2 * np.sum(Y * np.dot(X, beta_1.T), axis = 0) + \
              np.dot(np.sum(X**2, axis = 0), beta_2.T) + \
              2 * np.sum(np.sum((beta_1[:, :p-1][:, np.newaxis, :] * X[:, :p-1]) \
                  * ((np.cumsum(((X * beta_1[:, np.newaxis])[:,:,1:])[:, :, ::-1],axis = 2))[:, :, ::-1]), axis = 1), axis = 1)
    ...
    for t in np.arange(1,T):
        for k in np.arange(t):
            rho_mu[k,t] = (inv_sigma2_1[t]) * rho_sigma2[k,t] * \
                          (np.dot(U[:,t], U[:,k]) - \
                          np.sum(np.dot(U[:,k], np.delete(U[:,:t], k, axis=1)) * np.delete(rho_mu[:t,t], k)))
...
def ELBO_F_sigma2(nu, T, tau_1, log_tau_1, a_star_sigma2, b_star_sigma2, inv_sigma2_1, log_inv_sigma2_1):
    ...
    ELBO_F_sigma_t = ((nu - T + np.arange(1,T+1,1)) / 2) * (log_tau_1 - np.log(2)) - \
                     ...
                     np.log(sy.special.gamma(a_star_sigma2)) + \
